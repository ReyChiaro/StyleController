...
class QwenImageEditPlusPipeline(DiffusionPipeline, QwenImageLoraLoaderMixin):
    ...
    def prepare_latents(
        ...
        if images is not None:
            if not isinstance(images, list):
                images = [images]
            all_image_latents = []
            for image in images:  # [C,_,H,W]
                image = image.to(device=device, dtype=dtype)
                if image.shape[1] != self.latent_channels:
                    image_latents = self._encode_vae_image(image=image, generator=generator)  # [C,_,H,W]
                else:
                    image_latents = image
    ...
    def __call__(
        ...
        if image is not None and not (isinstance(image, torch.Tensor) and image.size(1) == self.latent_channels):
            ...
            vae_images = []
            for img in image:
                # Resolution adaptive resize for textual and visual branch
